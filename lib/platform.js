...
class ShellyPro3EMPlatform {
  constructor(log, config, api) {
    ...
    this.logReadings = this.config.logReadings !== false;
  ...
  async poll() {
    try {
      ...
      const readingLog = this.logReadings ? this.log.info.bind(this.log) : this.log.debug.bind(this.log);
      readingLog(
        'Readings: Base Load=%s W, Surplus=%s W, Absolute Grid Flow=%s W.',
        readings.import,
        readings.export,
        readings.net,
      );
